...
export class PaymentsService {
  ...
  async payment(
    ...
    const paymentParams = {
      ...
      payment_success_at: params.payment_success_at,
    };

    const create = this.paymentRepository.create(paymentParams);
    try {
      const save = await this.paymentRepository.save(create);
      return save;
    } catch (error) {
